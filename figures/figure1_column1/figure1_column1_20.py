...
if __name__ == "__main__": 
	...
	# load the 1D dataset
	with open('../data/1D/1D_20.pkl', 'rb') as f:
	...
	no_inducing = 15
	varGP = variational_GP(train_inputs.data.numpy(), np.expand_dims(train_outputs.data.numpy(),1), no_inducing=no_inducing)
	...
	full_pred_mean, full_pred_covar = fullGP.joint_posterior_predictive(train_inputs, train_outputs, test_inputs, noise=False)
	...
	#plt.fill_between(torch.squeeze(test_inputs).data.numpy(), var_pred_mean + 2*var_pred_sd, var_pred_mean - 2*var_pred_sd, color='b', alpha=0.3)
	plt.plot(torch.squeeze(test_inputs).data.numpy(), var_pred_mean, color='b')
	...
	#plt.fill_between(torch.squeeze(test_inputs).data.numpy(), full_pred_mean + 2*full_pred_sd, full_pred_mean - 2*full_pred_sd, color='b', alpha=0.3)
	plt.plot(torch.squeeze(test_inputs).data.numpy(), full_pred_mean + 2*full_pred_sd, color='r', linestyle='--')
	plt.plot(torch.squeeze(test_inputs).data.numpy(), full_pred_mean - 2*full_pred_sd, color='r', linestyle='--')

	# save plot

	filepath = 'var_vs_fullGP_20.pdf'
