...
class variational_GP(nn.Module):  
    def __init__(self, Xn, Yn, no_inducing=15, kernel='SE', freeze_hyperparam=False, logsigmaf2=None, logl2=None, logsigman2=None):   # the GP takes the training data as arguments, in the form of numpy arrays, with the correct dimensions        
        ...
        self.Xn = torch.tensor(Xn).type(torch.double)
        self.Yn = torch.tensor(Yn).type(torch.double)
        self.no_inputs = Xn.shape[1]
        self.logsigmaf2 = nn.Parameter(torch.zeros(1, dtype=torch.double)) # function variance
        self.logl2 = nn.Parameter(torch.zeros(self.no_inputs, dtype=torch.double)) # horizontal length scales
        ...
        if freeze_hyperparam == True:
            self.logsigmaf2 = nn.Parameter(torch.tensor(logsigmaf2, dtype=torch.double))
            self.logl2 = nn.Parameter(torch.tensor(logl2, dtype=torch.double))
            self.logsigman2 = nn.Parameter(torch.tensor(logsigman2, dtype=torch.double))
        ...
        self.jitter_factor = 1e-10
        # 10^-5 jitter used for the Boston dataset
        #self.jitter_factor = 1e-5

        # initialise inducing points randomly for the Boston dataset
        self.Xm = nn.Parameter(torch.Tensor(Xn[random.sample(range(Xn.shape[0]),no_inducing),:]).type(torch.double))

        # initialise inducing points in a small interval for the toy 1D dataset
        #upper = 3
        #lower = 2
        #input_locations = (lower - upper) * torch.rand(no_inducing,self.Xn.shape[1], dtype=torch.double) + upper
        #self.Xm = nn.Parameter(input_locations.type(torch.double),requires_grad=True)
    ...
    def get_K(self, input1, input2):
        if self.kernel == 'SE':
            ...
            K = torch.exp(self.logsigmaf2) * torch.exp(-torch.Tensor([np.sqrt(3)]).type(torch.double)*torch.sum(scaled_distances, 0) + torch.sum(torch.log(1 + torch.Tensor([np.sqrt(3)]).type(torch.double)*scaled_distances), 0))
        else:
