...
LL = pd.read_table('./LL_results_not_frozen.tsv', header=None)
LL_mean = LL.mean(1).values
LL_sem = LL.sem(1).values
LL_PP = pd.read_table('./sebastian/LML_results_PP_unfrozen.tsv', header=None)
LL_PP_mean = LL_PP.mean(1).values
LL_PP_sem = LL_PP.sem(1).values
LL_SPGP = pd.read_table('./sebastian/LML_results_SPGP_unfrozen.tsv', header=None)
...
plt.ylim([-600,425])
plt.xlim([0,455])
plt.yticks([-600,-400,-200,0, 200, 400])
plt.ylabel('Log marginal likelihood')
plt.xlabel('Number of inducing points')

# save plot

filepath = 'var_LL_not_frozen.pdf'
